...
<CardGroup cols={2}>
  ...
  <Card title="Kubernetes and Helm" icon="dharmachakra" href="/installation/kubernetes-helm">
  ...
  <Card title="Upgrading from v1" icon="arrow-up-right-dots" href="/installation/migration">
...
2. **Configure the providers you want.** Most platforms need you to register a
   ...
   See [Provider API keys](/providers/overview).
3. **Put a reverse proxy in front of it.** Postiz sets secure cookies, so it
   expects HTTPS in production. See [Reverse proxies](/reverse-proxies/caddy).

Once you can log in, the rest of the product works the same as it does on
cloud. Head to [Schedule your first post](/quickstart).
