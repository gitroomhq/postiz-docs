...
The build step is the most memory-intensive part. If `pnpm install` or
`pnpm build` OOMs on a small VM, bump the Node heap:

```bash
NODE_OPTIONS="--max-old-space-size=4096" pnpm install
```

For production: 2 GB RAM is the absolute floor and only works if you
run Postgres/Redis externally. Below that you'll hit OOM kills under load.
