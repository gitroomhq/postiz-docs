...
| `attachments` | array | Uploaded media URLs. A video can also be an object with `path` and `thumbnail`, where `thumbnail` is the path of an uploaded JPEG/PNG used as the video cover (e.g. Instagram Reel cover) |
