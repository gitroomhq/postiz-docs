...
- In the "Service Stack" tab;
 ...
 - (Optional) Change the Domain in the "Settings" for the `postiz` service to your HTTPS Domain (keep the :5000 port).
- In the "Storages" tab, check that the 3 related volumes are created.
- In the "Logs" tab, check that the "`postiz`, `postiz-postgres` and `postiz-redis` have no issues.
