...
description: 'Let users authorize your app to access Postiz on their behalf'
---
...
sequenceDiagram
    ...
    User->>YourApp: Clicks "Connect with Postiz"
    YourApp->>Postiz: Redirect to /oauth/authorize
    Postiz->>User: Shows consent screen
    User->>Postiz: Approves access
    Postiz->>YourApp: Redirects with ?code=...
    YourApp->>Postiz: POST /oauth/token (exchange code)
    Postiz->>YourApp: Returns access_token
    YourApp->>Postiz: API calls with access_token
```
...
<Steps>
  <Step title="Register Your OAuth App">
    Go to **Settings > Developers > Apps** in your Postiz dashboard and create an OAuth application.
  ...
  <Step title="Handle the Callback">
    After the user makes a decision, Postiz redirects them to your **Redirect URL** with query parameters.
