...
There is no dedicated duplicate / clone endpoint. To copy an existing post, fetch it with `GET /posts` and submit the returned content as a new `POST /posts` request.
