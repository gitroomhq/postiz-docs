...
To set a custom cover for a Reel, upload a JPEG/PNG image via the
[uploads endpoint](/public-api/uploads/upload-file) and pass its path as the
`thumbnail` of the video media object. Without a `thumbnail`, Instagram picks
a frame from the video as the cover.
