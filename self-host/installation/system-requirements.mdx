...
VM running everything on one host (see [Docker Compose](/self-host/installation/docker-compose)).
...
If you'd rather build from source or use Kubernetes, see [Docker Compose](/self-host/installation/docker-compose), [Docker](/self-host/installation/docker), or [Kubernetes / Helm](/self-host/installation/kubernetes-helm).
...
same files through `/uploads/:path*`. See [Uploads & storage](/self-host/configuration/uploads).

If you'd rather offload media: configure [Cloudflare R2](/self-host/configuration/r2).
...
ports is the typical setup. See [Reverse Proxies](/self-host/reverse-proxies/caddy).
