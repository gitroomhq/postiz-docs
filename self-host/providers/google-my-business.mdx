...
The `/integrations/social/gmb` page can surface session-replay overlay errors that mask the actual OAuth flow. The handshake usually still completes. Refresh the Channels page after authorising on Google's side. See [Known Issues](/general/troubleshooting/known-issues).
