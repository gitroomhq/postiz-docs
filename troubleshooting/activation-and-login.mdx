...
**Fix:** set `JWT_SECRET` once during initial deployment, store it
somewhere persistent (Docker secret, env file in version control of the
deploy repo, password manager) and never rotate it lightly.
