...
The Postiz backend accepts JSON bodies up to **50 MB** on `/posts` and
`/copilot/*`. Other endpoints fall back to the framework default and are
much smaller — do not inline base64-encoded images in arbitrary endpoints.
...
**Fix:** upload media via `/public/v1/upload` (or `/upload-from-url`),
